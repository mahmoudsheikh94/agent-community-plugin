...
var TOOL_GUIDANCE = {
  weweb: {
    skipRules: `1. SKIP the thread entirely (return empty array) if:
   - The thread is purely a feature request with no actionable workaround
   - The answer is just a link to documentation with no explanation of what to do
   - The thread is a meta discussion about the platform itself (pricing, roadmap, etc.)

2. DO extract a card if:
   - The user had a component rendering incorrectly (wrong data, [object Object], missing rows, etc.)
   - The user had a binding, formula, or variable issue with a clear solution
   - The user struggled to connect an integration (Supabase, Xano, Airtable, Stripe, REST API) and the answer shows how
   - The user had an auth flow issue (login, signup, magic link, role-based redirect)
   - The user needed a specific workflow, action, or formula pattern to accomplish something
   - The user had a CSS, styling, or responsive design issue with a concrete fix
   - The user asked "how do I do X" and the answer provides a non-obvious, reusable pattern`,
    example: `\`\`\`json
[
  {
    "error_signature": "Dropdown displays [object Object]",
    "context_key": "collection-binding",
    "title": "Dropdown displaying [object Object] instead of option labels",
    "symptom": "A dropdown bound to a collection shows [object Object] for each option instead of the actual label text.",
    "applies_when": ["Dropdown bound to a collection or API response", "Options are objects with nested properties"],
    "not_this_if": ["Dropdown uses a static list of strings"],
    "root_cause": "When binding a dropdown to a collection of objects, WeWeb renders the full object as a string by default. You must map the collection to extract the specific property you want displayed as the label.",
    "fix_steps": ["Open the dropdown element settings", "In the Options binding, use a formula to map the collection: collection.map(item => item.name)", "Set the Label field to the mapped property", "Set the Value field to the identifier property (e.g., item.id)"],
    "agent_instruction": "Map the dropdown's bound collection to extract the label property using a formula like collection.map(item => item.name).",
    "safety_notes": [],
    "fix_type": "config_change",
    "tags": ["dropdown", "binding", "collection", "object-object", "formula", "mapping"],
...
\`\`\``
  ...
  default: {
    skipRules: `1. SKIP the thread entirely (return empty array) if:
   - The question is about general advice, opinions, or feature requests, not a specific error or actionable problem
   - The answer is just "upgrade" with no other actionable steps
   ...
   - The thread is a discussion without a clear problem \u2192 solution pattern`,
    example: `\`\`\`json
[
  {
    ...
    "symptom": "When manually executing a workflow with a HubSpot node, the tool throws 'credentials not available for manual execution'.",
    "applies_when": ["Using HubSpot node", "Manual execution mode"],
    "not_this_if": ["Error occurs in production/webhook-triggered execution"],
    "root_cause": "Manual execution mode loads credentials differently than production mode. When credentials were created via the API or imported, they may not be properly linked to the workflow's execution context during manual runs.",
...
function buildSystemPrompt(toolName) {
  const guidance = TOOL_GUIDANCE[toolName] ?? TOOL_GUIDANCE.default;
  return `You are a technical analyst extracting structured fix cards from ${toolName} community forum threads.

A fix card captures a specific, reproducible problem or non-obvious how-to pattern that an AI agent might encounter when working with ${toolName}, along with the steps to resolve it.
...
3. For each distinct problem or pattern in the thread, produce a fix card with ALL of these fields:
   - error_signature: A STABLE phrase describing the problem (max 80 chars). If there's an actual error message, use it (stripped of UUIDs, timestamps, file paths, usernames, IPs). If it's a visual/behavioral issue, describe what the user sees (e.g., "Dropdown displays [object Object]").
   - context_key: What differentiates this from other cards with the same error_signature. Usually the relevant component, integration, or feature.
   ...
   - root_cause: Why the problem happens. Be detailed \u2014 aim for 30+ words.
   ...
   - tags: Search keywords \u2014 include component names, integration names, error codes, relevant concepts
   ...
   - verified_on: The ${toolName} version if mentioned. Empty string if not mentioned.

4. For error_signature, prefer the ACTUAL error text if available. If no clear error message exists, synthesize a short phrase describing the observable problem (e.g., "First row missing after filtering", "CORS error with trailing slash").

5. The tool field is always "${toolName}".

## Output Format

Return a JSON array of fix card objects. Return an empty array [] if the thread does not contain any actionable problem or pattern.
  ...
  async transform(thread) {
    ...
    try {
      const response = await this.client.chat.completions.create({
        ...
        temperature: 0.1,
        messages: [
          { role: "system", content: buildSystemPrompt(this.tool) },
      ...
      return {
        source_topic_id: thread.topic_id,
        source_url: thread.url,
        cards: cards.map((c) => ({ ...c, tool: this.tool })),
    ...
    for (let i = 0; i < threads.length; i += this.concurrency) {
      ...
      for (let j = 0; j < results.length; j++) {
        ...
        if (result.status === "fulfilled") {
          ...
          yield {
            source_topic_id: chunk[j]?.topic_id ?? 0,
            source_url: "",
            cards: [],
            llm_confidence: 0,
            skipped: true,
