...
async function searchCards(tool, limit = 200) {
  const sb = getSupabaseClient();
  if (!sb) return [];
  let query = sb.from("fix_cards").select("*").order("quality_score", { ascending: false }).limit(limit);
  if (tool) {
...
async function loadCardSupabase(id) {
  const sb = getSupabaseClient();
  if (!sb) return null;
  const { data, error } = await sb.from("fix_cards").select("*").eq("id", id).single();
  if (error) return null;
  return data;
}
async function listToolsSupabase() {
  const sb = getSupabaseClient();
  if (!sb) return [];
  const { data, error } = await sb.from("fix_cards").select("tool").limit(1e3);
  if (error) return [];
  const tools = new Set((data ?? []).map((row) => row.tool));
  return [...tools];
}
