...
var DiscourseCrawler = class {
  constructor(baseUrl, rateLimiter = new RateLimiter(), categoryId, categorySlug) {
  ...
  async fetchTopicPage(page) {
    ...
    const url = this.categoryId && this.categorySlug ? `${this.baseUrl}/c/${this.categorySlug}/${this.categoryId}/l/latest.json?page=${page}` : `${this.baseUrl}/latest.json?page=${page}`;
...
// src/ingest/tool-config.ts
var TOOL_CONFIGS = {
  n8n: {
    name: "n8n",
    displayName: "n8n",
    forumBaseUrl: "https://community.n8n.io",
    categorySlug: "questions",
    questionsCategoryId: 12,
    rateLimitPerWindow: 20,
    rateLimitWindowMs: 1e4
  },
  weweb: {
    name: "weweb",
    displayName: "WeWeb",
    forumBaseUrl: "https://community.weweb.io",
    categorySlug: "ask-us-anything",
    questionsCategoryId: 6,
...
function getToolConfig(tool) {
  ...
  if (!config) {
    const known = Object.keys(TOOL_CONFIGS).join(", ");
...
loadEnvFile();
function migrateOldLayout(tool) {
  if (tool !== "n8n") return;
  const oldState = resolve("data", "ingest", "state.json");
  const newDir = resolve("data", "ingest", tool);
  if (existsSync(oldState) && !existsSync(join(newDir, "state.json"))) {
    const oldDir = resolve("data", "ingest");
    mkdirSync(newDir, { recursive: true });
    renameSync(oldState, join(newDir, "state.json"));
    const oldProcessed = join(oldDir, "processed.jsonl");
    if (existsSync(oldProcessed)) {
      renameSync(oldProcessed, join(newDir, "processed.jsonl"));
    }
    const oldRaw = join(oldDir, "raw");
    if (existsSync(oldRaw)) {
      cpSync(oldRaw, join(newDir, "raw"), { recursive: true });
    }
    console.log(`Migrated ingest data from flat layout to data/ingest/${tool}/`);
  }
}
function getIngestDir(tool) {
  migrateOldLayout(tool);
  return resolve("data", "ingest", tool);
}
function loadCrawlState(tool) {
  const statePath = join(getIngestDir(tool), "state.json");
...
function saveCrawlState(tool, state) {
  const dir = getIngestDir(tool);
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "state.json"), JSON.stringify(state, null, 2) + "\n");
}
function saveRawThread(tool, thread) {
  const rawDir = join(getIngestDir(tool), "raw");
...
function loadProcessedIds(tool) {
  const path = join(getIngestDir(tool), "processed.jsonl");
  if (!existsSync(path)) return /* @__PURE__ */ new Set();
  const lines = readFileSync(path, "utf-8").split("\n").filter(Boolean);
  return new Set(lines.map((l) => parseInt(l, 10)));
}
function loadRawThreads(tool) {
  const rawDir = join(getIngestDir(tool), "raw");
  if (!existsSync(rawDir)) return [];
  const processedIds = loadProcessedIds(tool);
  const threads = [];
...
async function handleCrawl(args) {
  const tool = getFlag(args, "--tool") ?? "n8n";
  const config = getToolConfig(tool);
  const maxTopics = parseInt(getFlag(args, "--max-topics") ?? "0") || void 0;
  const verbose = args.includes("--verbose");
  const state = loadCrawlState(tool);
  const crawler = new DiscourseCrawler(
    config.forumBaseUrl,
    new RateLimiter(config.rateLimitPerWindow, config.rateLimitWindowMs),
    config.questionsCategoryId,
    config.categorySlug
  );
  console.log(`Starting ${config.displayName} crawl from page ${state.last_page}...`);
...
async function handleTransform(args) {
  ...
  const verbose = args.includes("--verbose");
  const threads = loadRawThreads(tool);
  if (threads.length === 0) {
    console.log(
  ...
  console.log(`Estimated LLM cost: ~$${estimatedCost} (${model})`);
  if (dryRun) {
    console.log("Dry run \u2014 no cards will be saved.");
  }
  const { ThreadTransformer } = await import("./thread-to-cards-CZTZVB6S.js");
  const { Deduplicator } = await import("./dedup-Z24BMIOH.js");
  const { validateCard, generateCardId } = await import("./validation-MHUGEGHH.js");
  const { saveCard, rebuildToolIndex } = await import("./store-KNSW3IQ6.js");
  ...
  let deduped = 0;
  let failed = 0;
  const processedPath = join(getIngestDir(tool), "processed.jsonl");
...
async function handleStatus(args) {
  ...
  const processedCount = loadProcessedIds(tool).size;
...
async function handleStats(args) {
  const tool = getFlag(args, "--tool") ?? "n8n";
  getToolConfig(tool);
  const runsDir = join(getIngestDir(tool), "runs");
...
async function handleImport(args) {
  ...
  if (!topicId) {
    console.error("Usage: agent-community ingest import --tool <name> --topic <ID>");
    process.exit(1);
  }
  const crawler = new DiscourseCrawler(
    config.forumBaseUrl,
  ...
  saveRawThread(tool, thread);
  console.log(`Saved raw thread to data/ingest/${tool}/raw/${topicId}.json`);
  const apiKey = process.env.OPENAI_API_KEY;
  if (apiKey) {
    console.log(`
Transforming with LLM...`);
    const { ThreadTransformer } = await import("./thread-to-cards-CZTZVB6S.js");
    const transformer = new ThreadTransformer(apiKey, { concurrency: 1, tool });
...
async function handlePush(args) {
  ...
  const { upsertCards } = await import("./supabase-74PKH7AN.js");
  const cardsDir = join(resolve("data"), "tools", tool, "cards");
...
async function handleUpdate(args) {
  ...
  if (!tool) {
    console.error("Usage: agent-community ingest update --tool <name>");
    process.exit(1);
  }
  getToolConfig(tool);
  const verbose = args.includes("--verbose");
  const maxTopics = getFlag(args, "--max-topics");
  console.log(`
=== ${tool} incremental update ===
`);
  const crawlArgs = ["--tool", tool];
  if (verbose) crawlArgs.push("--verbose");
  if (maxTopics) crawlArgs.push("--max-topics", maxTopics);
  console.log("Step 1/2: Crawling new topics...");
  await handleCrawl(crawlArgs);
  console.log("\nStep 2/2: Transforming new threads...");
  const transformArgs = ["--tool", tool, "--concurrency", "3"];
  if (verbose) transformArgs.push("--verbose");
  await handleTransform(transformArgs);
  console.log(`
...
async function handleIngestCommand(subcommand, args) {
  switch (subcommand) {
    ...
    default:
      console.log(`Usage: agent-community ingest <command> [--tool <name>]

All commands accept --tool <name> (default: n8n). Known tools: n8n, weweb

Commands:
  crawl      [--tool <name>] [--max-topics N] [--verbose]              Crawl solved topics from forum
  transform  [--tool <name>] [--model <model>] [--dry-run] [--min-quality N] [--concurrency N] [--verbose]  Transform raw threads
  run        [--tool <name>] [--max-topics N] [--dry-run] [--min-quality N]  Full pipeline: crawl + transform
  import     [--tool <name>] --topic <ID>                              Import a single topic by ID
  push       [--tool <name>]                                           Push all local cards to Supabase
  update     --tool <name> [--max-topics N] [--verbose]                 Incremental update: crawl new + transform
  stats      [--tool <name>] [--last N]                                Show pipeline run history
  status     [--tool <name>]                                           Show current crawl state`);
  }
}
export {
  handleIngestCommand
