...
A fix card captures a specific, reproducible error that an AI agent might encounter when using ${toolName}, along with the steps to resolve it.
...
2. For each distinct error in the thread, produce a fix card with ALL of these fields:
   ...
   - context_key: What differentiates this from other cards with the same error_signature. Usually the relevant component, node, or integration name.
...
3. For error_signature, ALWAYS prefer the ACTUAL error text from the thread. If no clear error message exists, synthesize a short phrase that an AI agent would see in the output.

4. The tool field is always "${toolName}".
